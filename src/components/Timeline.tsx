...
return (
    <div className={`flex relative my-2.5 w-1/2 ${isOdd ? 'self-end justify-start pl-[30px]' : 'justify-end pr-[30px]'}`}>
        <div className={`shadow-md rounded-[5px] bg-white flex flex-col p-[15px] relative w-[400px] max-w-[70%] ${isOdd ? 'text-left items-start' : 'text-right items-end'}`}>
            ...
            <span 
                className={`text-white text-xs font-bold tracking-wider px-1.5 py-1 absolute top-1.5 uppercase ${isOdd ? 'right-1.5' : 'left-1.5'}`}
            ...
            <p className="text-base leading-6 my-[15px] max-w-[250px]">{data.text}</p>
        </div>
    </div>
)
